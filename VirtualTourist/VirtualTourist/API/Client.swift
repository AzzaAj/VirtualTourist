...
class Client {
    ...
    func searchBy(latitude: Double, longitude: Double, totalPages: Int?, completion: @escaping (_ result: PhotosParser?, _ error: Error?) -> Void) {
        ...
        var page: Int {
            if let totalPages = totalPages {
                let page = min(totalPages, 4000/40)
    ...
    func downloadIm(imageUrl: String, result: @escaping (_ result: Data?, _ error: NSError?) -> Void) {
        ...
        let task = session.dataTask(with: request as URLRequest) { (data, response, error) in
            ...
            if let error = error {
                ...
                if (error as NSError).code == URLError.cancelled.rawValue {
                    ...
                    sendError("There was an error with your request. Check the Internet connection: \(error.localizedDescription)")
                }
                return
            }
